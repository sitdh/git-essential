...
\begin{frame}{Exercise II: Add files to index}
    ...
    \only<4->{
        \begin{enumerate}[\$]
            \item<4-> git add .
            \item<5-> git commit -m \textbf{'Add ignore list'} .gitignore 
            \item<5-> git commit -m \textbf{'Add composer config'} composer.json
            \item<6-> git commit -am \textbf{'Add testing lib and config'}
                \begin{enumerate}[-]
                    \item<7-> \em{phpunit.xml}
                    \item<7-> \em{phpunit.phar}
                \end{enumerate}
        \end{enumerate}
    }
\end{frame}
